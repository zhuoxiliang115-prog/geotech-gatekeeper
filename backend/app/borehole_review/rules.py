...
# "CZ" isn't in the AECOM §3.15 table captured in
# reference/borehole-log-standard.md - found by the corpus sweep (2 real
# instances, Heathcote.pdf DBH39: "CZ, 30 mm thick") as a genuine,
# undocumented-here code, left unrecognised (safely excluded from spacing/
# seam-content, not guessed at) until confirmed. The user has since
# directly confirmed CZ is the same thing as CS (Crushed Seam/Crushed
# Zone) - not an inference drawn from the two codes' similar names, an
# actual answer - so it's grouped with CS accordingly below and in
# classification.py's _SEAM_TYPE_CODES.
